...
public class LoginController {
	...
	public String goLogins(Model model, MdUser user, HttpSession session, HttpServletRequest request,
		...
		if(list.size()==1){//登陆成功
//			if("已登陆".equals(((MdUser)list.get(0)).getMdStdname())){
//				model.addAttribute("user", ((MdUser)list.get(0)).getMdStdname());
//				return "redirect:/login";
//			}
			
			((MdUser)list.get(0)).setMdStdname("已登陆");
//			int i=userService.update(list.get(0), null);
			...
			session.setAttribute("user", (MdUser)list.get(0));
			session.setAttribute("autoLogin", autoLoginCookie);
			return "redirect:/to_index";
		}else{
			return "redirect:/login";
		}
	...
	public String to_index(Model model,HttpServletRequest request){
		//查询游记分页显示
